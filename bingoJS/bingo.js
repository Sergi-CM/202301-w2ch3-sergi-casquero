const greeting = () => {
  ...
  alert(
    "Empiezas la partida con 1.000 puntos. En cada turno perderás 10, así que cuantos menos turnos necesites para completar el cartón, más puntos ganarás. ¡Suerte!"
  );
};
...
const confirmCard = () => {
  const confirm = prompt("¿Quieres cambiar los números del cartón? s/n");
  switch (confirm) {
    case "s":
...
const mainNumber = () => {
  const number2 = numbersArray2[Math.floor(Math.random() * 99)];
  if (isNaN(number2)) {
    mainNumber();
  } else {
...
const newGame = () => {
  const newG = prompt("¿Quieres volver a jugar? s/n");
  switch (newG) {
    case "s":
    ...
    default:
      alert(
        "Sólo di 's' o 'n'. Encontrarás estas letras por el teclado, seguro ;)"
      );
      newGame();
  }
};
...
const nextTurn = () => {
  const next = confirm("¿Quieres sacar un número?");
  if (next != true) {
