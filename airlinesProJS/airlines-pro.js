...
const interfaz = () => {
    ...
    for(x = 0; x < flights.length; x++) {
        if (flights[x].scale) {
            console.log(`El vuelo con origen ${flights[x].from} y destino ${flights[x].to}, tiene un coste de ${flights[x].cost}€ y SÍ realiza escala`); 
        } else {
...
const perfil = () => {
    ...
    case "admin":
                ...
                case "s":
                    ...
                    const nuevos = () => {
                        let nuevoVuelo = {
                            ...
                            scale: prompt("Escribe si el vuelo hace escala (TRUE) o no (FALSE)")
                        }
        ...
        switch (pregunta2) {
            case "s":
                    const eliminar = () => {
                        let vueloElim = Number(prompt("Escribe el ID del vuelo que quieras eliminar."));
