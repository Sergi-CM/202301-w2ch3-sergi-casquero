...
function checkAnswer() {
  const userAnswer = prompt().toLocaleLowerCase();
...
function endKeyword() {
  const byeOptions = [
    "Si todo lo dejas a medias no estarán muy contentos en casa...",
    "No me digas eso de 'voy a por tabaco y vuelvo'.",
    "¿Tan malo es el juego? :'(",
    "Da igual, vete, no me importa.\n\n\n\nYa te echo de menos...",
  ];
...
// Volver a jugar
function newGame() {
  const new2 = prompt("¿Quieres volver a jugar? s/n");
  switch (new2) {
    case "s":
      reset();
      startGame();
