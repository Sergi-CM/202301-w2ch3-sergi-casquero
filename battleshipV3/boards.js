...
const generateRandomShips = (ship) => {
    ...
    let randomRow = direction ? Math.floor(Math.random() * global.rows + 1) : Math.floor(Math.random() * (global.rows - ship.totalHealth) + 1);
    let randomCol = direction ? Math.floor(Math.random() * (global.cols - ship.totalHealth) + 1) : Math.floor(Math.random() * global.cols + 1);
