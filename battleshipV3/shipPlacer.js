  ...
  if (visible) {
    ...
    for (let i = 0; i < ships.length; i++) {
      ...
      horizontalButton.onclick = () =>
        directionButtons(
          `${ships[i].shipName}-horizontal`,
          `${ships[i].shipName}-vertical`
        );
...
const placePlayerShips = (
  ...
  const shipxCoordinate = parseInt(
    document.querySelector("." + yCoordinate).value
  );
  const shipyCoordinate = parseInt(
    document.querySelector("." + xCoordinate).value
  );
  const horizontalDirection = Boolean(
    document
      .querySelector("." + horizontal)
      .classList.contains("ship-button-clicked")
  );
  const shipComment = document.querySelector("." + comment);
  let isPlaceable;
  ...
  if (!horizontalDirection && shipxCoordinate > 11 - ship.totalHealth) {
    shipComment.textContent = "Can't place the ship outside the grid.";
    ship.isPlaced = false;
    return;
  }

  if (shipxCoordinate === 0 || shipyCoordinate === 0) {
    shipComment.textContent = "Please enter values from 1 to 10.";
    ship.isPlaced = false;
    return;
  }

  isPlaceable = checkEmptySpaces(
    shipxCoordinate,
    shipyCoordinate,
    horizontalDirection,
    ship
  ...
  if (horizontalDirection) {
    for (let i = 0; i < ship.totalHealth; i++) {
      ...
      document
        .querySelector(
          ".player-" + shipxCoordinate + "-" + (shipyCoordinate + i)
        )
        .classList.add("player-ship");
      document.querySelector(
        ".player-" + shipxCoordinate + "-" + (shipyCoordinate + i)
      ).textContent = ship.letter;
    ...
    for (let i = 0; i < ship.totalHealth; i++) {
      ...
      document
        .querySelector(
          ".player-" + (shipxCoordinate + i) + "-" + shipyCoordinate
        )
        .classList.add("player-ship");
      document.querySelector(
        ".player-" + (shipxCoordinate + i) + "-" + shipyCoordinate
...
const startGame = () => {
  const allPlaced = checkIfAllPlaced();
