...
const showShipsHealth = (player) => {
    ...
    let playerToMap = player === "player" ? global.playerShips : global.enemyShips
    ...
    playerToMap.map((ship) => {
        ...
        if (ship.hits === ship.totalHealth) {
            ship.isSunk = true;
            document.querySelector(`.${player}-${ship.shipName}`).setAttribute("src", "./assets/" + ship.shipName + "2.png");
        } 
        ...
        for (let i = 0; i < ship.totalHealth; i++) {
            ...
            if (i < ship.hits) {
                ...
                document.querySelector(`.${player}-${ship.shipName}-health`).appendChild(healthPoint)
            } else {
