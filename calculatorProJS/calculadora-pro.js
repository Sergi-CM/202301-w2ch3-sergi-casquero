...
const operandos = () => {
  do {
    var numeros = Number(
      prompt(
        "Escribe un operando. Déjalo vacío cuando no quieras introducir más."
      )
    );
    if (isNaN(numeros)) {
...
const operaciones = (listaNum) => {
  if (listaNum.length === 2) {
    const raiz = Math.sqrt(listaNum[0]);
    const raizDecim = raiz.toFixed(3);
...
const nuevaOperacion = () => {
  const respuesta = prompt("¿Quieres hacer otra operación? s/n");
  switch (respuesta) {
    case "s":
